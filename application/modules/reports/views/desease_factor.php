...
<form id="search_report" method="get" action="reports/desease_overall" style="padding:10px; border:1px solid #ccc; margin-bottom:10px;">
	...
	<div>
		...
		<?
			...
			echo form_dropdown('main_factor',$main_factor_array,'age','id="main_factor" class="span3"');
		?>
		...
		<?php
			...
			echo form_dropdown('second_factor',$second_factor_array_1,'','id="second_factor_1" class="span3"');
			echo form_dropdown('second_factor',$second_factor_array_2,'','id="second_factor_2" class="span3"');
		?>
...
$sql = "SELECT
				(
					...
					FROM
						childrens
					WHERE title = 'ด.ช.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 0
				) male_year_0,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ช.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 1
				) male_year_1,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ช.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 2
				) male_year_2,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ช.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 3
				) male_year_3,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ช.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 4
				) male_year_4,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ช.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 5
				) male_year_5,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ช.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 6
				) male_year_6,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ช.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 7
				) male_year_7,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ญ.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 0
				) female_year_0,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ญ.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 1
				) female_year_1,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ญ.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 2
				) female_year_2,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ญ.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 3
				) female_year_3,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ญ.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 4
				) female_year_4,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ญ.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 5
				) female_year_5,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ญ.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 6
				) female_year_6,
				(
					SELECT
						count(id)
					FROM
						childrens
					WHERE title = 'ด.ญ.' AND TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) = 7
				) female_year_7";
